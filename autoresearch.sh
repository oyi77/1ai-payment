...
# (forwarder.service.test.ts is excluded: it stubs fetch but performs REAL
# DNS lookups via the SSRF guard, which would make the metric
# network-dependent).
set -uo pipefail
...
# Collect offline-safe unit test files (exclude DNS-dependent forwarder test).
