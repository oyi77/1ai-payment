...
export async function createRefund(params: CreateRefundParams): Promise<Refund> {
  ...
  if (order.merchant_id !== params.merchant_id && order.project_id !== params.merchant_id) {
    throw new GatewayError('', 'Order does not belong to this merchant');
  }

  // Verify order is in a refundable state
  if (order.status !== 'success') {
    throw new GatewayError('', `Cannot refund order with status: ${order.status}`);
  }

  // Calculate refund amount (default to full order amount)
  const refundAmount = params.amount ?? order.amount;
  if (refundAmount > order.amount) {
    throw new GatewayError('', `Refund amount (${refundAmount}) exceeds order amount (${order.amount})`);
  }
  ...
  if (gateway?.refundPayment && order.gateway_reference) {
    try {
      const result = await gateway.refundPayment(order.gateway_reference, refundAmount);
      gatewayRefundId = result.gatewayRefundId;
      status = result.status;
    } catch (err: unknown) {
      if (err instanceof GatewayError && err.message.includes('REFUND_NOT_SUPPORTED')) {
  ...
  await db.execute({
    sql: `INSERT INTO refunds (id, order_id, merchant_id, amount, gateway, gateway_refund_id, status, reason)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [id, params.order_id, params.merchant_id, refundAmount, order.gateway, gatewayRefundId, status, params.reason ?? null],
  });
  ...
  logger.info('Refund created', { id, order_id: params.order_id, amount: refundAmount, status });
...
export async function getRefundById(id: string): Promise<Refund | null> {
  ...
  return result.rows.length > 0 ? mapRefundRow(result.rows[0] as Record<string, unknown>) : null;
}

export async function listRefunds(merchantId: string, limit = 20, offset = 0): Promise<{ refunds: Refund[]; total: number }> {
  const db = getDb();
  ...
  return {
    refunds: result.rows.map((row) => mapRefundRow(row as Record<string, unknown>)),
    total,
  };
}
