...
import { healthResponseSchema, defaultHook } from '../schemas';
...
const healthRoute = createRoute({
  ...
  description: 'Returns service status, database connectivity, and per-gateway configuration status.',
  security: [], // No auth required
  responses: {
    200: {
      description: 'Health status (always 200; inspect `status` field for degraded state)',
...
healthRoutes.openapi(healthRoute, async (c) => {
  ...
  const gatewayStatus = Object.fromEntries(
    Object.entries(gateways).map(([name, status]) =>
