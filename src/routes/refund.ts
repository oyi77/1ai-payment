...
import { createRefund, listRefunds, getRefundById } from '../services/refund.service';
...
const createRefundRoute = createRoute({
  ...
  description: 'Refunds a successful payment. Partial refund supported. Gateway must support refunds or refund is marked pending for manual processing.',
  security: [{ ApiKeyAuth: [] }],
  request: {
    body: {
  ...
  responses: {
    201: {
      ...
      content: {
        'application/json': {
          schema: z.object({ success: z.literal(true), data: refundResponseSchema }),
        },
      },
    },
