...
export class MidtransGateway implements PaymentGateway {
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    ...
    const baseUrl = config.MIDTRANS_ENVIRONMENT === 'production' ? PRODUCTION_URL : SANDBOX_URL;
    ...
    if (params.customerName) (body.customer_details as Record<string, string>).first_name = params.customerName;
    if (params.customerEmail) (body.customer_details as Record<string, string>).email = params.customerEmail;
    ...
    if (!response.ok) {
      ...
      throw new GatewayError('midtrans', `Charge failed: ${response.status} ${error}`);
    }
  ...
  private extractStatus(payload: MidtransCallbackPayload): PaymentStatus {
    ...
    if (transaction_status === 'capture' && fraud_status === 'accept') return 'success';
