...
export async function verifySignature(
	...
	const baseUrl =
		config2.PAYPAL_ENVIRONMENT === "production"
...
export function normalizeEvent(
	...
	return {
		...
		amount: Math.round(Number.parseFloat(resource.amount.value) * 100), // Convert to cents
...
export function extractStatus(body: unknown): PaymentStatus {
	...
	if (eventType === "CHECKOUT.ORDER.APPROVED" && status === "approved")
		return "pending";
