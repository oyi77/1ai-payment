...
import type { EscrowStatus, EscrowAttestation, CreateEscrowParams } from './types';

/**
 * Parse and verify an escrow attestation
 */
export function parseAttestation(body: unknown): { attestation: EscrowAttestation; error?: string } {
  try {
    ...
    const attestation: EscrowAttestation = {
      ...
      approved: Boolean(data.approved || data.status === 'approved' || data.status === 'completed'),
      signature: data.signature ? String(data.signature) : undefined,
      notes: data.notes ? String(data.notes) : undefined,
    };

    if (!attestation.escrowId) return { attestation, error: 'Missing escrow_id' };
    if (!attestation.evaluator) return { attestation, error: 'Missing evaluator' };
 ...
 */
export function escrowStatusToPaymentStatus(status: EscrowStatus): 'success' | 'pending' | 'failed' | 'expired' | 'cancelled' | 'refunded' {
  switch (status) {
...
export function normalizeEvent(
  ...
  return {
    ...
    gateway_reference: String(data.tx_hash || data.attestation_hash || data.gateway_reference || ''),
