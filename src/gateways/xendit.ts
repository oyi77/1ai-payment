...
export class XenditGateway implements PaymentGateway {
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    ...
    const baseUrl = config.XENDIT_ENVIRONMENT === 'production' ? PRODUCTION_URL : SANDBOX_URL;

    // Determine payment type from paymentMethod
    const isVA = ['bca', 'mandiri', 'bni', 'bri', 'permata', 'cimb', 'btn', 'bjb', 'bsi'].includes(
    ...
    if (isVA) {
      ...
      if (!response.ok) {
        ...
        throw new GatewayError('xendit', `VA Create failed: ${response.status} ${error}`);
      }
      ...
      return {
        ...
        paymentUrl,
        expiresAt: result.expiry_date ? new Date(result.expiry_date).toISOString() : undefined,
      };
    } else {
      ...
      if (!response.ok) {
        const error = await response.text();
        throw new GatewayError('xendit', `Invoice Create failed: ${response.status} ${error}`);
      }
  ...
  normalizeEvent(body: unknown, metadata?: Record<string, unknown> | null): NormalizedPaymentEvent {
    const payload = body as XenditInvoiceCallbackPayload | XenditVACallbackPayload;
  ...
  private extractStatus(payload: XenditInvoiceCallbackPayload | XenditVACallbackPayload): PaymentStatus {
