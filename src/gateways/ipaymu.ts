...
export class IPaymuGateway implements PaymentGateway {
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    ...
    if (!config.IPAYMU_API_KEY || !config.IPAYMU_VA_KEY) {
      throw new GatewayError('ipaymu', 'IPAYMU_API_KEY or IPAYMU_VA_KEY not configured');
    }
    ...
    if (!response.ok) {
      ...
      throw new GatewayError('ipaymu', `Create failed: ${response.status} ${error}`);
    }

    const result = (await response.json()) as IPaymuCreateResponse;

    if (result.Status !== 200 || !result.Data) {
      throw new GatewayError('ipaymu', result.Message || 'Payment creation failed');
    }

    return {
      gatewayReference: result.Data.SessionID,
      paymentUrl: result.Data.PaymentURL,
      expiresAt: result.Data.ExpiredAt ? new Date(result.Data.ExpiredAt).toISOString() : undefined,
    };
  }
  ...
  verifySignature(body: unknown, _headers: Record<string, string>): boolean {
    ...
    const expected = crypto
      .createHash('sha256')
      .update(`${config.IPAYMU_VA_KEY}${payload.order_id}${payload.status}${payload.amount}${config.IPAYMU_API_KEY}`)
      .digest('hex');
