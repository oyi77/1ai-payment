...
export class TripayGateway implements PaymentGateway {
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    ...
    const baseUrl = config.TRIPAY_ENVIRONMENT === 'production' ? PRODUCTION_URL : SANDBOX_URL;
    ...
    if (!response.ok) {
      ...
      throw new GatewayError('tripay', `Create failed: ${response.status} ${error}`);
    }

    const result = (await response.json()) as TripayCreateResponse;

    if (!result.success || !result.data) {
      throw new GatewayError('tripay', result.message || 'Payment creation failed');
    }
